...
- 数据库使用 PostgreSQL 17，本地和容器环境不使用 H2 等替代数据库。
- 异步任务使用 PostgreSQL 任务表持久化，Spring Boot 应用内定时 worker 通过数据库锁领取任务；本期不使用 Redis、RabbitMQ、Kafka 或其他消息中间件。
- Excel 使用 Apache POI `SXSSF` 流式生成，文件写入本地挂载目录；文件元数据和任务状态保存在 PostgreSQL。
- 合成订单通过可重复执行的初始化脚本生成，初始化后只读。

### 本地运行与交付

- 本地运行使用 Docker Compose，包含前端、后端和 PostgreSQL。默认对外端口为前端 `5173`、后端 `8080`、PostgreSQL `5432`；端口冲突时通过环境变量覆盖。
...
数据库任务队列让异步任务状态具备持久性，且比引入独立消息中间件更适合当前学习范围。单应用 worker 不面向生产级水平扩展，但数据库领取必须避免同一任务被重复执行，并继续遵守幂等约束。
